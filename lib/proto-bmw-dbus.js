...
function clear_all(message, callback = null) {
	...
	log.msg(proto[module_name].queue_input.length + ' ' + message + ' - resets: ' + status.intf[module_name].errors.resets + ', errors: ' + status.intf[module_name].errors.current);
...
function error_increment(message, callback = null) {
	...
	log.msg(message + ' - new error count: ' + status.intf[module_name].errors.current);
...
function process() {
	let queue_process = proto[module_name].queue_input;
...
function create(msg) {
	...
	let buffer = Buffer.alloc((msg.msg.length + proto[module_name].buffer_offset));
