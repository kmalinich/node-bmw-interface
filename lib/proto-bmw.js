...
function clear_all(message) {
	...
	log.msg({ msg : message+' - resets: '+status.intf.errors.resets+', errors: '+status.intf.errors.current });
...
function error_reset() {
	if (status.intf.errors.current !== 0) {
		status.update('intf.errors.current', 0);
		log.msg({ msg : 'Errors resolved' });
	}
}
...
function error_increment(message) {
	...
	log.msg({ msg : message+' - new error count: '+status.intf.errors.current });
}
...
function parser() {
	...
	if (process_return.failed === false) {
		...
		if (status.intf.messages % 100 === 0) {
			log.msg({ msg : 'Message count: '+status.intf.messages });
		}
	}
...
function process() {
	...
	if (app_intf != 'kbus' || msg.dst.name != 'GLO') {
		socket.bus_rx(msg);
	}
