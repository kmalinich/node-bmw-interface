...
function text(data, callback = null) {
	...
	check(() => {
		...
		log.msg('Text: \'' + data.upper + ' ' + data.lower + '\'');

		// This data needs to be put in status variable like MID text
		// tder string + trim to 16 chars
		data.upper = data.upper.substring(0, 16).padEnd(16);
		data.lower = data.lower.substring(0, 16).padStart(16);
	...
	intf.intf.port.set(intf.opts.open, (error) => {
		...
		command('on', null, () => {
			command('brightness', 0xFF, () => {
				command('contrast', 0xB0, () => {
					command('autoscroll-off', null, () => {
						command('home', null, () => {
							command('clear', null, () => {
								text({
									upper : 'bmwi@lcd',
...
function send(buffer, waiter = false, callback = null) {
	...
	if (!status.intf.up) {
		log.msg('Waiting for port to open');
		return false;
	}
	...
	intf.intf.port.write(buffer, (error) => {
		...
		if (intf.intf.draining + intf.intf.writing === 0) {
			...
			intf.intf.port.drain((error) => {
				...
				else { intf.config.debug && log.msg('Drain success'); }
