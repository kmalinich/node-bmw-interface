...
function intf_config(pass, fail) {
	...
	socket.interfaces[app_type].bind(get_url(app_type), (error) => {
		if (error) {
			log.msg({ msg : 'zeroMQ server bind '+error });
		...
		if (app_type == 'client') {
			...
			Object.keys(socket.interfaces).forEach((socket_intf) => {
				if (socket_intf !== app_type) return;
...
function event_config(pass, fail) {
	Object.keys(socket.interfaces).forEach((socket_intf) => {
		...
		socket.interfaces[socket_intf].on('connect', () => {
			log.msg({ msg : 'Peer connected' });
		});

		// When a peer disconnects
		socket.interfaces[socket_intf].on('disconnect', () => {
			log.msg({ msg : 'Peer disconnected' });
		});
		...
		socket.interfaces[socket_intf].on('message', (topic, message) => {
			log.msg({ msg : topic+' message received via '+socket_intf });
			// Decode AMP message
			on_message(message);
		});
	});
	...
	socket.interfaces[app_type].on('unbind', () => {
		...
		// Reset basic vars
		if (app_type == 'client') json.status_reset_basic();
	});
...
function term(pass, fail) {
	...
	if (!status.server.connected) {
		log.msg({ msg : 'zeroMQ server already unbound' });
		process.nextTick(pass);
		return true;
	}
	...
	socket.interfaces[app_type].unbind(get_url(app_type), (error) => {
		if (error) {
			log.msg({ msg : 'zeroMQ server unbind '+error });
