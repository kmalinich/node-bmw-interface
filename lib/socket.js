...
function get_url(target) {
	...
	console.dir(zmq_conf, { showHidden: true, depth: null, colors: true});

	return zmq_conf.proto+'://'+zmq_conf.host+':4001';
}
...
function send(event, data) {
	...
	socket.intf.send([ app_type+'-tx', amp_message.toBuffer() ]);
}
...
function recv(message) {
	...
	switch (data.event) {
		case 'bus-tx' :
			if (data.data.bus === app_type) bus.data.send(data.data); break;
...
function event_config(pass, fail) {
	...
	log.msg({ msg : 'Configured event listeners' });
...
function intf_config(pass, fail) {
	...
	log.msg({ msg : 'Connecting interface' });
	socket.intf.connect(get_url('client'));
...
function term(pass, fail) {
	...
	switch (status.server.connected) {
		case true:
			...
			break;
		case false:
			log.msg({ msg : 'zeroMQ dealer socket already disconnected' });
	}
