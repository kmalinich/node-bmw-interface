...
function error_fmt(error) {
	let error_string = app_intf;
...
async function init_intf() {
	try {
		...
		if (existing) {
			...
			log.lib('Removed leftover socket');
		}
	}
	catch (error) {
		switch (error.errno) {
			...
			default : {
				log.lib('Error during socket fs.statSync()');
				log.error(error_fmt(error));
	...
	try {
		...
		log.lib('Error: failed to open socket');
		log.error(error_fmt(error));
		process.exit(5);
	}
} // async init_intf()
