...
function send(event, data) {
	...
	socket.intf.send([ app_intf + '-tx', amp_message.toBuffer() ]);
}
...
function recv(message) {
	...
	switch (data.event) {
		case 'bus-tx' :
			if (data.data.bus === app_intf) bus.data.send(data.data); break;
...
function intf_config(pass, fail) {
	...
	log.msg({ msg : 'Set ' + get_type(app_intf) + ' identity: \'' + app_intf + '\'' });
