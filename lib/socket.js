...
function on_data(data) {
	...
	switch (data.event) {
		case 'bus-tx' :
			if (data.data.bus === app_type) bus_data.send(data.data);
			break;

		case 'bus-rx' :
			if (app_type == 'client') bus_data.receive(data.data.bus);
			break;
		...
		case 'lcd-color'   :
		case 'lcd-command' :
		case 'lcd-text'    : on_lcd_data(data.event, data.data); break;
...
// Handle incoming lcd-data messages
function on_lcd_data(event, data) {
	if (app_type !== 'lcd') return;
	...
	switch (event) {
		case 'lcd-color'   : interface.lcd.color(data.values);
		case 'lcd-command' : interface.lcd.command(data.cmd, data.value);
		case 'lcd-text'    : interface.lcd.text(data);
	}
}
