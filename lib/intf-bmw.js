...
function send(message) {
	...
	console.dir({
		function : 'send',
		length   : intf.intf.queue_write.length,
		// writing  : intf.intf.writing,
		// message  : message,
		// buffer   : buffer,
		// queue    : intf.intf.queue_write,
	});
...
function write() {
	...
	let write_len = intf.intf.queue_write.length;
	...
	setTimeout(() => {
		...
		// Remove number of buffered elements from the end of the write queue
		intf.intf.queue_write.splice(write_len, intf.intf.queue_write.length);
