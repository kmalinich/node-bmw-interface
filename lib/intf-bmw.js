...
async function send(message) {
	...
	await intf.intf.queue.push(queueItem);

	log.lib(`qLen: ${intf.intf.queue.length} :: ${queueItem.src}>${queueItem.dst} :: aC: ${queueItem.attemptCount} :: Queued`);
...
async function write() {
	...
	if (queueItem.writing === true) {
		// log.lib(`qLen: ${intf.intf.queue.length} :: ${queueItem.src}>${queueItem.dst} :: aC: ${queueItem.attemptCount} :: Writing in progress`);
		return;
	}

	// log.lib(`qLen: ${intf.intf.queue.length} :: ${queueItem.src}>${queueItem.dst} :: aC: ${queueItem.attemptCount} :: Writing start`);

	queueItem.writing = true;
	queueItem.attemptCount++;

	if (queueItem.attemptCount > config.options.attemptLimit[app_intf]) {
		log.error(`qLen: ${intf.intf.queue.length} :: ${queueItem.src}>${queueItem.dst} :: aC: ${queueItem.attemptCount} :: Failed after ${attemptLimit} attempts to send message`);
