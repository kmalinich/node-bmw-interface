...
async function send(message) {
	...
	if (intf.intf.writeInterval === null) {
		intf.intf.writeInterval = setInterval(write, config.options.attemptInterval[app_intf]);
	}
} // async send(message)

async function write() {
	if (intf.intf.queue.length === 0) {
		log.lib(`qLen: ${intf.intf.queue.length} :: Writing complete`);
		clearInterval(intf.intf.writeInterval);
		intf.intf.writeInterval = null;
		return;
	}

	const queueItem = intf.intf.queue[0];

	if (queueItem.writing === true) {
		// log.lib(`qLen: ${intf.intf.queue.length} :: ${queueItem.src} > ${queueItem.dst} :: Writing in progress`);
		return;
	}

	log.lib(`qLen: ${intf.intf.queue.length} :: ${queueItem.src} > ${queueItem.dst} :: Writing start`);

	queueItem.writing = true;
	...
	if (config.options.ctsrts_retry[app_intf] === true) {
		...
		try {
			...
			if (modemStatus.cts !== true) {
				if (queueItem.attemptCount > config.options.attemptLimit[app_intf]) {
					log.error(`qLen: ${intf.intf.queue.length} :: ${queueItem.src} > ${queueItem.dst} :: Failed after ${attemptLimit} attempts to send message with cts: true`);
					await intf.intf.queue.shift();
					return;
				}

				log.lib(`qLen: ${intf.intf.queue.length} :: ${queueItem.src} > ${queueItem.dst} :: Reattempting, attemptCount ${queueItem.attemptCount} (after cts: false) to send message with cts: true`);
				queueItem.writing = false;
				return;
			}
		}
		catch (serialportGetError) {
			log.lib(`qLen: ${intf.intf.queue.length} :: ${queueItem.src} > ${queueItem.dst} :: Reattempting, attemptCount ${queueItem.attemptCount} (after serialportGetError) to send message with cts: true`);
		...
		if (queueItem.attemptCount > 1) {
			log.lib(`qLen: ${intf.intf.queue.length} :: ${queueItem.src} > ${queueItem.dst} :: Attempt ${queueItem.attemptCount} to send message with cts: true`);
		}
	}
	...
	try {
		...
		log.lib(`qLen: ${intf.intf.queue.length} :: ${queueItem.src} > ${queueItem.dst} :: Wrote`, queueItem.frameData);

		// Only log after multiple attempts
		// if (queueItem.attemptCount > 1) {
		// 	log.lib(`qLen: ${intf.intf.queue.length} :: ${queueItem.src} > ${queueItem.dst} :: Wrote after ${queueItem.attemptCount} attempts`);
		// }
		...
		log.lib(`qLen: ${intf.intf.queue.length} :: ${queueItem.src} > ${queueItem.dst} :: Reattempting, attemptCount ${queueItem.attemptCount} (after serialportWriteError) to send message`);
...
async function init() {
	...
	try {
		...
		error_out('configuring interface', error);
		return false;
	}
