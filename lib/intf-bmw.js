...
async function configure_port() {
	...
	intf.intf.port = await new SerialPort({ path : intf.path, ...intf.opts.init });
...
async function write(frameData, attemptCount = 0) {
	...
	try {
		...
		// Only log after multiple attempts
		if (attemptCount > 1) {
			log.lib(`Wrote after ${attemptCount} attempts, src: ${hex.i2s(frameData[0])}, dst: ${hex.i2s(frameData[2])}`);
		}
