...
async function write() {
	if (intf.intf.queue.length === 0) {
		log.lib(`Writing complete, current queue length ${intf.intf.queue.length}`);
	...
	if (queueItem.writing === true) {
		log.lib(`Writing in progress, src: ${queueItem.src}, dst: ${queueItem.dst}, current queue length ${intf.intf.queue.length}`);
		return;
	}

	log.lib(`Writing start, src: ${queueItem.src}, dst: ${queueItem.dst}, current queue length ${intf.intf.queue.length}`);
	...
	if (config.options.ctsrts_retry[app_intf] === true) {
		...
		try {
			...
			if (modemStatus.cts !== true) {
				if (queueItem.attemptCount > config.options.attemptLimit[app_intf]) {
					log.error(`Failed after ${attemptLimit} attempts to send message with cts: true, src: ${queueItem.src}, dst: ${queueItem.dst}`);
					await intf.intf.queue.shift();
					return;
				}

				log.lib(`Reattempting, attemptCount ${queueItem.attemptCount} (after modemStatus.cts === false) to send message with cts: true, src: ${queueItem.src}, dst: ${queueItem.dst}`);
			...
			log.lib(`Reattempting, attemptCount ${queueItem.attemptCount} (after serialportGetError) to send message with cts: true, src: ${queueItem.src}, dst: ${queueItem.dst}`);
			queueItem.writing = false;
			return;
		}
		...
		if (queueItem.attemptCount > 1) {
			log.lib(`Attempt ${queueItem.attemptCount} to send message with cts: true, src: ${queueItem.src}, dst: ${queueItem.dst}`);
		}
	}
	...
	try {
		...
		log.lib(`port.write() wrote message, src: ${queueItem.src}, dst: ${queueItem.dst}`, queueItem.frameData);

		// Only log after multiple attempts
		if (queueItem.attemptCount > 1) {
			log.lib(`Wrote after ${queueItem.attemptCount} attempts, src: ${queueItem.src}, dst: ${queueItem.dst}`);
		}
