...
async function write(frameData, attemptCount = 0) {
	...
	try {
		...
		if (modemStatus.cts !== true) {
			...
			console.error('Queuing attempt %o to send message (after modemStatus.cts !== true) with cts: true, src: %s, dst: %s', attemptCount, frameData[0], frameData[2]);
		...
		console.error('Queuing attempt %o to send message (after serialportGetError) with cts: true, src: %s, dst: %s', attemptCount, frameData[0], frameData[2]);
		await new Promise(resolve => setTimeout(resolve, attemptInterval));
		// Line below had await, removed it for testing
		await write(frameData, attemptCount);
		return true;
