...
function error_out(message, error) {
	const error_fmt = error.toString().replace(/Error: /, '')
		.replace(/Error: /, '')
		.trim();
...
function send(message) {
	...
	const buffer = proto.proto.create(message);
...
function write() {
	...
	intf.intf.port.write(write_buf, (error) => {
		...
		// After drain, re-kick it with a delay assumed based on 9600 baud
		// intf.intf.port.drain(() => { setTimeout(write, (1 / 9.6)); });
	});
}
